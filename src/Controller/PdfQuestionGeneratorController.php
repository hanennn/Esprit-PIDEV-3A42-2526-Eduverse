...
class PdfQuestionGeneratorController extends AbstractController
{
    ...
    public function generateFromPdf(Request $request): JsonResponse
    {
        ...
        if (!is_string($projectDir)) {
            return $this->json(['ok' => false, 'message' => 'kernel.project_dir invalide.'], 500);
        }
        ...
        $converted = mb_convert_encoding($out, 'UTF-8', 'UTF-8, ISO-8859-1, Windows-1252');
        if ($converted === false) {
            return $this->json(['ok' => false, 'message' => 'Erreur encodage UTF-8.'], 500);
        }
        ...
        if (!is_array($data)) {
            return $this->json([
                'ok' => false,
                'message' => 'JSON invalide renvoyé par Python',
                'raw' => $converted,
            ], 500);
        }

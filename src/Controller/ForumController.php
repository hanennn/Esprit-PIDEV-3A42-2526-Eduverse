...
class ForumController extends AbstractController
{
        ...
        if ($user) {
            ...
            if ($form->isSubmitted() && $form->isValid()) {
                ...
                $this->historiqueLogger->log(
                    ...
                    'create',
                    $message->getId(),
                    sprintf('Message #%d ajouté sur le sujet #%d "%s".', $message->getId(), $sujet->getId(), $sujet->getTitre() ?? ''),
                    $user
                );
                $entityManager->flush();
    ...
    public function deleteMessage(int $id, Request $request, EntityManagerInterface $entityManager): Response
    {
        ...
        $isAuthor = $message->getAuteur()->getId() === $user->getId();
        $isSujetOwner = $sujet->getAuteur()->getId() === $user->getId() && in_array('ROLE_TEACHER', $user->getRoles());

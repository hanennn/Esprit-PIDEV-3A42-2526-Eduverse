...
public function toggleActive(Request $request, User $user, EntityManagerInterface $entityManager): Response
{
    ...
    if (!$this->isCsrfTokenValid('toggle_active_' . $user->getId(), $request->request->getString('_token'))) {
        $this->addFlash('error', 'Token invalide.');
        return $this->redirectToRoute('app_profile');
    }
...
public function updateProfile(Request $request, EntityManagerInterface $entityManager): Response
{
    ...
    if ($request->request->has('nom')) {
            $parts = explode(' ', trim($request->request->getString('nom')), 2);
            $user->setNom($parts[0]);
    ...
    if ($request->request->has('specialite')) {
        $user->setSpecialite($request->request->getString('specialite'));
        
    }
    if ($request->request->has('username')) {
        $user->setUsername($request->request->getString('username'));
    }
    if ($request->request->has('experience')) {
        $user->setExperience($request->request->getString('experience'));
    }
    
    // Handle password change
    if ($request->request->has('password') && !empty($request->request->get('password'))) {
        $hashedPassword = password_hash($request->request->getString('password'), PASSWORD_BCRYPT);

...
class ProfileController extends AbstractController
{
    ...
    ): Response
    {
        ...
        if (in_array('ROLE_ADMIN', $roles) || in_array('ROLE_SUPER_ADMIN', $roles)) {
            ...
            $currentFilter = (string) $request->query->get('filter', 'all');
            $sortBy = (string) $request->query->get('sort', 'date_desc');
            ...
            foreach ($quizzes as $quiz) {
                // FIX 1: removed always-true ternary, getCertifications() never returns null
            ...
            // COURSES DATA
            // ----------------------------
            $courseSearch = (string) $request->query->get('search', '');
            $courseSearchCriteria = (string) $request->query->get('search_criteria', 'titre');
            $courseSort = (string) $request->query->get('sort', 'id');
            ...
            $forumHistoriques = $historiqueRepository->findLatest(50);
            ...
            return $this->render('backoffice.html.twig', [
                ...
                'forumHistoriques' => $forumHistoriques,
            ]);
        }
        // Check TEACHER
        elseif (in_array('ROLE_TEACHER', $roles)) {
            ...
            foreach ($instructorQuizzes as $quiz) {
                ...
                foreach ($certs as $cert) {
                    $totalScore += $cert->getScoreObtenu();
                }
            }
            ...
            foreach ($instructorQuizzes as $quiz) {
                ...
                $passedCount = 0;
             foreach ($certs as $cert) {
                $score = (float) ($cert->getScoreObtenu() ?? 0);

                $quizScore += $score;

                // si ton seuil de réussite est 50
                if ($score >= 50) {
                    $passedCount++;
                }
             }
    ...
    ): Response
    {
        ...
        try {
            // Get form data
            $titre = $request->request->getString('titre_cours') ?: null;
            $niveau = $request->request->getString('niv_cours') ?: null;
            $matiere = $request->request->getString('matiere_cours') ?: null;
            $langue = $request->request->getString('langue_cours') ?: null;
            $description = $request->request->has('description') ? $request->request->getString('description') : null;

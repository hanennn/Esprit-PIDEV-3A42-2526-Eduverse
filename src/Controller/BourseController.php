...
class BourseController extends AbstractController
{
    ...
    ): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            /** @var \Symfony\Component\HttpFoundation\File\UploadedFile $lettreFile */
            ...
            if ($lettreFile) {
                ...
                try {
                    $lettreFile->move(
                        $this->getParameter('kernel.project_dir') . '/public/uploads/cancelatures',
                        $newFilename
                    );
    ...
    ): Response
    {
        ...
        if ($this->isCsrfTokenValid('annuler' . $demande->getId(), $request->request->get('_token'))) {

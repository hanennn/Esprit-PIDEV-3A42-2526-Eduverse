...
$quiz = $certification->getQuiz();

if (!$user instanceof User) {
    throw new \RuntimeException("Utilisateur manquant sur la certification.");
}
if (!$quiz instanceof Quiz) {
    throw new \RuntimeException("Quiz manquant sur la certification.");
}

$certificationFinale->setUser($user);
$certificationFinale->setQuiz($quiz);
...
private function sendCertificationEmail(CertificationFinale $certificationFinale, MailerInterface $mailer): void
{
    ...
    if (!$user instanceof User) {
        throw new \RuntimeException("User manquant pour l'envoi email.");
    }

    $to = $user->getEmail();
    if (!is_string($to) || $to === '') {
        throw new \RuntimeException("Email utilisateur invalide.");
    }

    $email = (new TemplatedEmail())
        ->from('eduversee2026@gmail.com')
        ->to($to)
...
{
    $secret = (string) ($_ENV['APP_CERT_SECRET'] ?? '');

    $id = $cert->getId();
    $user = $cert->getUser();
    $date = $cert->getDateEmission();

    if (!is_int($id)) {
        throw new \RuntimeException('ID certificat manquant.');
    }
    if (!$user instanceof User) {
        throw new \RuntimeException('User manquant pour token.');
    }

    $email = $user->getEmail();
    if (!is_string($email) || $email === '') {
        throw new \RuntimeException('Email user manquant pour token.');
    }

    if (!$date instanceof \DateTimeInterface) {
        throw new \RuntimeException('Date emission manquante pour token.');
    }

    $data = $id . '|' . $email . '|' . $date->format('Y-m-d H:i:s');
   ...
   private function generateAiSummary(CertificationFinale $cert, HttpClientInterface $httpClient): ?string
{
    ...
    if (!is_string($apiKey) || $apiKey === '') return null;

    $user = $cert->getUser();
    $quizEntity = $cert->getQuiz();
    $dateEmission = $cert->getDateEmission();

    if (!$user instanceof User) return null;
    if (!$quizEntity instanceof Quiz) return null;
    if (!$dateEmission instanceof \DateTimeInterface) return null;

    $prenom = (string) $user->getPrenom();
    $nom = (string) $user->getNom();
    $name = trim($prenom . ' ' . $nom);

    $quiz = (string) $quizEntity->getTitre();

    $score = $cert->getTentative()?->getScoreObtenu();
    $badge = $cert->getTentative()?->getBadge();
    $date = $dateEmission->format('d/m/Y');

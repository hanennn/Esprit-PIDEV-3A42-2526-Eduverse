...
class EventAdminController extends AbstractController
{
    ...
    public function add(Request $request, EntityManagerInterface $entityManager, \Symfony\Component\Validator\Validator\ValidatorInterface $validator): Response
    {
        ...
        if ($request->isMethod('POST')) {
            ...
            // Conditional fields
            if ($type === 'webinaire' && is_string($lienWebinaire)) {
                $event->setLienWebinaire($lienWebinaire);
            }
            if ($type === 'challenge' && is_string($niveau)) {
            ...
            if (is_string($heureDebStr)) {
                try {
                    $event->setHeureDeb(new \DateTime($heureDebStr));
                } catch (\Exception $e) { }
            }
            if (is_string($heureFinStr)) {
                try {
    ...
    public function editInscription(Request $request, EntityManagerInterface $entityManager, int $id): Response
    {
        ...
        if ($request->isMethod('POST')) {
            ...
            if (empty($errors)) {
                $inscription->setStatut((string)$statut);
                $inscription->setNote(is_string($note) ? $note : null);
                $entityManager->flush();
                ...
                return $this->redirectToRoute('app_event_inscriptions', ['id' => $event->getId()]);
            }
        }
    ...
    public function deleteInscription(Request $request, EntityManagerInterface $entityManager, int $id): Response
    {
        ...
        if ($this->isCsrfTokenValid('delete_inscription'.$inscription->getId(), (string)$request->request->get('_token'))) {

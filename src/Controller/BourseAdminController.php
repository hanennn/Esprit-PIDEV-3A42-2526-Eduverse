...
class BourseAdminController extends AbstractController
{
            ...
            if (count($violationList) > 0) {
                ...
                if ($imageFile) {
                    ...
                    try {
                        ...
                        $imageFile->move(
                            $projectDir.'/public/uploads/bourses',
                            $newFilename
                        );
    ...
    {
        if ($this->isCsrfTokenValid('delete'.$bourse->getId(), (string)$request->request->get('_token'))) {
    ...
    public function editDemande(Request $request, EntityManagerInterface $entityManager, int $id, \Symfony\Component\Mailer\MailerInterface $mailer): Response
    {
        ...
        if ($request->isMethod('POST')) {
            ...
            if (empty($errors)) {
                $demande->setStatut((string)$statut);
                $demande->setNiveauEtudes((string)$niveauEtudes);
                $demande->setNote(is_string($note) ? $note : null);
                $entityManager->flush();
                ...
                $bourse = $demande->getBourse();
                if (!$bourse) {
                    throw $this->createNotFoundException('Bourse non trouvée');
                }
                return $this->redirectToRoute('app_bourse_applications', ['id' => $bourse->getId()]);
            }
        }
    ...
    public function deleteDemande(Request $request, EntityManagerInterface $entityManager, int $id): Response
    {
        ...
        $bourseId = $bourse->getId();

        if ($this->isCsrfTokenValid('delete_demande'.$demande->getId(), (string)$request->request->get('_token'))) {

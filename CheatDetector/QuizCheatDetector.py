...
cheat_count   = 0
exam_over     = False
cooldown      = 0  # frames to wait before auto-starting next window
...
with FaceLandmarker.create_from_options(options) as landmarker:
    while True:
        ...
        # If exam is over, show final message and wait for Q
        if exam_over:
            overlay = frame.copy()
            ...
            cv2.putText(frame, f"You cheated {MAX_CHEATS} times. Exam terminated.",
                        (frame.shape[1]//2 - 270, frame.shape[0]//2 + 20),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            cv2.putText(frame, "Press Q to exit",
                        (frame.shape[1]//2 - 100, frame.shape[0]//2 + 60),
        ...
        if detection.face_landmarks:
            ...
            # Auto-start recording after cooldown
            ...
            if recording:
                ...
                if len(frame_buffer) >= WINDOW_SIZE:
                    ...
                    frame_buffer = []
                    cooldown     = 30  # ~1 second pause between windows

                    if label == 'cheat':
                        cheat_count += 1
                        result_text  = f"WARNING: Cheating detected! ({cheat_count}/{MAX_CHEATS})"
        ...
        # HUD
        if recording:
            cv2.putText(frame, f"Analyzing... {len(frame_buffer)}/{WINDOW_SIZE}",
                        (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 165, 255), 2)
        elif cooldown > 0:
            cv2.putText(frame, "Standby...",
                        (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (200, 200, 200), 2)

        cv2.putText(frame, f"Cheat attempts: {cheat_count}/{MAX_CHEATS}",
                    (10, frame.shape[0] - 15), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 2)

        if result_text:
            cv2.putText(frame, result_text,
                        (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.75, result_color, 2)
